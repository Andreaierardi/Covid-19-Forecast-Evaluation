...
benchmark_tensor = np.full((3, 5, 41), 0)
...
for model in forecast_models:
    for forecast_interval in range(1,max_for):
        ...
        for j in range(start, end+forecast_interval):
            benchmark_tensor[model_number, forecast_interval,
                             j] = TT['dr'][j]-TT[model_used][j]
    model_number = model_number+1

print(benchmark_tensor[0,])
